...
class Database
{
    ...
    {
        $this->link = mysqli_connect("localhost","root","","DoAn") or die ();
    ...
    public function countRowByQuery($sql)
    {
        $result = mysqli_query($this->link, $sql) or die("Lỗi Truy Vấn countRowByQuery----" .mysqli_error($this->link));
        $num = mysqli_num_rows($result);
        return $num;
    }
    ...
    public function fetchSqlNum($sql)
    {
        $result = mysqli_query($this->link,$sql) or die("Lỗi  truy vấn sql " .mysqli_error($this->link));
        if( $result)
        {
    ...
    public function fetchLimit($table , $query, $limit)
    {
        $sql  = "SELECT * FROM {$table} WHERE ";
        $sql .= $query;
        $sql .= "LIMIT {$limit}";
        $result = mysqli_query($this->link,$sql) or die("Lỗi  truy vấn fetchLimit " .mysqli_error($this->link));
        $data = [];
        if( $result)
        {
            while ($num = mysqli_fetch_assoc($result))
    ...
    public function searching($table, array $data) {
        $sql = "FROM {$table} ";
        $subSql = "";
        foreach ($data as $key => $value) {
            switch ($key) {
                case 'search':
                    $subSql .= "WHERE {$table}.slug LIKE '%{$value}%' "; 
                    break;
                case 'size':
                    if(!empty($value)) {
                        $tmp = '';
                    foreach ($value as $size) {
                        // Split array and insert query
                        $tmp .= "{$table}.{$size} > 0 OR ";
                    }
                      $tmp = substr($tmp, 0, -4);
                      $subSql .= " AND (" . $tmp . ") ";
                ...
                case 'tag':
                    ...
                    break;
                case 'price':
                    $subSql .= " AND product.price BETWEEN ". $value[0]." AND " . $value[1]; 
                    break;
            }

        }
        $sql .= $subSql;
        return array("count" => "SELECT COUNT(DISTINCT {$table}.id) " . $sql , "sql" => "SELECT DISTINCT {$table}.id, {$table}.* " . $sql . " ORDER BY {$table}.id DESC");

    }
    public function checkPass(array $data) {
        $sql = "SELECT id,fullname FROM user WHERE username LIKE BINARY '" . $data['username'] ."' AND passwd LIKE BINARY '". $data['passwd'] ."' ";
        $result = mysqli_query($this->link,$sql) or die("Lỗi  truy vấn fetchOne " .mysqli_error($this->link));
        return mysqli_fetch_assoc($result);
    }
}
